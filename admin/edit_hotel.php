...
require('connect.php');
$nama = $_GET['nama'];
$sqlu = "SELECT * FROM hotel WHERE id = '$nama'";
...
if(isset($_POST['upload'])){
        ...
        $sql = "SELECT * FROM hotel WHERE id = '$nama'";
        ...
        if (!empty(array_filter($_FILES['image']['name']))) {
            ...
            foreach($_FILES['image']['name'] as $id=>$val){
                ...
                if (empty($checkimage)) {
                        if(!empty($sqlVal)) {
                        $insert = $db->query("UPDATE hotel SET gambar ='$fileName', nama_hotel='$nama_hotel', telepon='$telepon', alamat='$alamat', standard='$harga_standar', deluxe='$harga_deluxe', superior='$harga_superior' WHERE id='$nama'");
                        if (!empty($datao)) {
                    ...
                    if(!empty($sqlVall)) {
                    $insert = $db->query("UPDATE wisata SET nama_hotel='$nama_hotel', telepon='$telepon', alamat='$alamat', standard='$harga_standar', deluxe='$harga_deluxe', superior='$harga_superior' WHERE id='$nama'");
            ...
            $harga_superior  =$_POST['harga_superior'];
            $harga_deluxe    =$_POST['harga_deluxe'];
            $insert = $db->query("UPDATE hotel SET nama_hotel='$nama_hotel', telepon='$telepon', alamat='$alamat', standard='$harga_standar', deluxe='$harga_deluxe', superior='$harga_superior' WHERE id='$nama'");
            $inserto = $db->query("UPDATE other SET nama='$nama_hotel' WHERE nama='$checknama'");
            if($insert) {
                if ($inserto) {
                   echo "<script type='text/javascript'>alert('File Succesfully Updated');</script>";
                }
...
if(isset($_POST['deletemain'])){
    ...
    unlink($img);
    $isi="";
    $sqlk = "UPDATE hotel SET gambar='$isi' WHERE id = '$nama'";
...
if(isset($_POST['dtagmain'])){
    $isi = "";
    $sqlk = "UPDATE hotel SET kategori='$isi' WHERE id = '$nama'";
    $res= mysqli_query($db,$sqlk);
    }
...
if(isset($_POST['adtag'])){
    $tags = $_POST['adtag'];
    $sqll = "SELECT * FROM hotel WHERE nama_hotel = '$checknama'";
    $resultt = mysqli_query($db,$sqll);
    $dataa = $resultt -> fetch_assoc();
    $kat =$dataa['kategori'];
    if (empty($kat)) {
        $sqlk =("UPDATE hotel SET kategori='$tags' WHERE nama_hotel='$checknama'");
        $res= mysqli_query($db,$sqlk);
    } else {
...
<body>
    ...
    ?>
            <div class="container-fluid">
                <form method="post" action="edit_hotel.php?nama=<?php echo $nama?>" enctype="multipart/form-data">
                        ...
                        <div class="row justify-content-center">                         
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <input type="text" class="form-control" name="harga_standar" value="<?= $data['standard']?>">
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <?php
                                    $tag = "wisata";
                                    $sqlu = "SELECT * FROM kategori WHERE kategori <> '$mt' AND kategori NOT IN (SELECT kategori FROM otherkategori WHERE nama='$objek') AND main IN ('akomodasi','other')";
                                    $results = mysqli_query($db,$sqlu);
                                    while ($row = mysqli_fetch_array($results)) {
                                    if (!empty($results)) { ?>
