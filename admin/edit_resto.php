...
$nama = $_GET['nama'];
require('connect.php');
$sqlu = "SELECT * FROM rumah_makan WHERE id = '$nama'";
...
if(isset($_POST['upload'])){
        ...
        $sql = "SELECT * FROM rumah_makan WHERE id = '$nama'";
        $resulta = mysqli_query($db,$sql);
        $datao = $resulta -> fetch_assoc();
        $checknamas = $datao['nama'];
        ...
        if (!empty(array_filter($_FILES['image']['name']))) {
            ...
            foreach($_FILES['image']['name'] as $id=>$val){
                ...
                if(in_array($fileType, $allowedFileType)){
                        if(move_uploaded_file($tempLocation, $targetFilePath) ){
                            $sqlVal = "('".$nama_resto."', '".$fileName."', '".$harga_max."', '".$harga_min."', '".$alamat."', '".$telepon."')";
                            $sqlVall ="('".$fileName."', '".$nama_resto."')";
                        } else {
                ...
                if (empty($checkimage)) {
                        if(!empty($sqlVal)) {
                        $insert = $db->query("UPDATE rumah_makan SET nama='$nama_resto', gambar ='$fileName',  harga_max='$harga_max', harga_min='$harga_min', alamat='$alamat', telepon='$telepon' WHERE id='$nama'");
                        if (!empty($datao)) {
                            $inserto = $db->query("UPDATE other SET nama='$nama_hotel' WHERE nama='$checknamas'");
                        }else{
                    ...
                    if(!empty($sqlVall)) {
                    $insert = $db->query("UPDATE rumah_makan SET nama='$nama_resto', harga_max='$harga_max', harga_min='$harga_min', alamat='$alamat', telepon='$telepon' WHERE id='$nama'");
            ...
            $insert = $db->query("UPDATE rumah_makan SET nama='$nama_resto', harga_max='$harga_max', harga_min='$harga_min', alamat='$alamat', telepon='$telepon' WHERE id='$nama'");
            $inserto = $db->query("UPDATE other SET nama='$nama_resto' WHERE nama='$checknama'");
            if($insert) {
                if ($inserto) {
                   echo "<script type='text/javascript'>alert('File Succesfully Updated');</script>";
                }
...
if(isset($_POST['deletemain'])){
    ...
    unlink($img);
    $isi="";
    $sqlk = "UPDATE rumah_makan SET gambar='$isi' WHERE id = '$nama'";
...
if(isset($_POST['dtagmain'])){
    $isi = "";
    $sqlk = "UPDATE rumah_makan SET kategori='$isi' WHERE id = '$nama'";
    $res= mysqli_query($db,$sqlk);
    }
...
if(isset($_POST['adtag'])){
    $tags = $_POST['adtag'];
    $sqll = "SELECT * FROM rumah_makan WHERE nama = '$checknama'";
    $resultt = mysqli_query($db,$sqll);
    $dataa = $resultt -> fetch_assoc();
    $kat =$dataa['kategori'];
    if (empty($kat)) {
        $sqlk =("UPDATE rumah_makan SET kategori='$tags' WHERE nama='$checknama'");
        $res= mysqli_query($db,$sqlk);
    } else {
...
<body>
    ...
    ?>
            <div class="container-fluid">
                <form method="post" action="edit_resto.php?nama=<?php echo $nama?>" enctype="multipart/form-data">
                    <div class="container" id="">
                        <div class="row justify-content-center">                         
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="mb-3">
                                    <label for="nama_resto" class="form-label">Nama Rumah Makan</label>
                                    <input type="text" class="form-control" name="nama_resto" value="<?= $data['nama']?>">
                                    </div>
                                    <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" name="alamat_resto" value="<?= $data['alamat']?>">
                                    </div>
                                    <div class="mb-3">
                                    <label for="harga_standar" class="form-label">Harga Tertinggi</label>
                                    <input type="text" class="form-control" name="harga_max" value="<?= $data['harga_max']?>">
                                    </div>
                                    <div class="mb-3">
                                    <label for="harga_deluxe" class="form-label">Harga Terendah</label>
                                    <input type="text" class="form-control" name="harga_min" value="<?= $data['harga_min']?>">
                                    </div>
                                    ...
                                    <?php
                                    $tag = "wisata";
                                    $sqlu = "SELECT * FROM kategori WHERE kategori <> '$mt' AND kategori NOT IN (SELECT kategori FROM otherkategori WHERE nama='$objek') AND main IN ('kuliner','other')";
                                    $results = mysqli_query($db,$sqlu);
                                    while ($row = mysqli_fetch_array($results)) {
                                    if (!empty($results)) { ?>
